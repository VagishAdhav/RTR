...
int initialise(void)
{
    ...
    // load textures
    if (loadGLTexture(&textureStone, "Stone.bmp") == False)
    {
        fprintf(gpFile, "loadGLTexture failed for Stone.bmp\n");
        return -2;
    }

    if (loadGLTexture(&textureKundali, "Kundali.bmp") == False)
    {
        fprintf(gpFile, "loadGLTexture failed for Kundali.bmp\n");
        return -3;
    }
