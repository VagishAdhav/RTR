...
int main(void)
{
    ...
    // set window title
    XStoreName(gpDisplay, window, "VVA:XWindow - Ortho Triangle");
...
int initialise(void)
{
    ...
    gIdentiyMatrix[0] = 1.0f;
    gIdentiyMatrix[1] = 0.0f;
    gIdentiyMatrix[2] = 0.0f;
    gIdentiyMatrix[3] = 0.0f;
    gIdentiyMatrix[4] = 0.0f;
    gIdentiyMatrix[5] = 1.0f;
    gIdentiyMatrix[6] = 0.0f;
    gIdentiyMatrix[7] = 0.0f;
    gIdentiyMatrix[8] = 0.0f;
    gIdentiyMatrix[9] = 0.0f;
    gIdentiyMatrix[10]= 1.0f;
    gIdentiyMatrix[11]= 0.0f;
    gIdentiyMatrix[12]= 0.0f;
    gIdentiyMatrix[13]= 0.0f;
    gIdentiyMatrix[14]= 0.0f;
    gIdentiyMatrix[15]= 1.0f;
...
void resize(int width, int height)
{
    ...
    if (width < height)
    {
        fLeft = -100.0f,
        fRight = 100.0f,
        fNear = -(100.0f * ((GLfloat)height/(GLfloat)width));
        fFar = (100.0f * ((GLfloat)height/(GLfloat)width));
    }
    else
    {
        fLeft = (-100.0f * ((GLfloat)width/(GLfloat)height));
        fRight = (100.0f * ((GLfloat)width/(GLfloat)height));
        fNear = -100.0f;
        fFar = 100.0f;
    }

    // fill up orthographic projection matrix
    orthoProjectionMatrix[0] = 2.0f/(fRight - fLeft);
    orthoProjectionMatrix[1] = 0.0f;
    orthoProjectionMatrix[2] = 0.0f;
    orthoProjectionMatrix[3] = 0.0f;

    orthoProjectionMatrix[4] = 0.0f;
    orthoProjectionMatrix[5] = 2.0f/(fTop - fBottom);
    orthoProjectionMatrix[6] = 0.0f;
    orthoProjectionMatrix[7] = 0.0f;

    orthoProjectionMatrix[8] = 0.0f;
    orthoProjectionMatrix[9] = 0.0f;
    orthoProjectionMatrix[10]= -(2.0f/(fFar - fNear));
    orthoProjectionMatrix[11]= 0.0f;

    orthoProjectionMatrix[12] = -((fRight + fLeft)/(fRight - fLeft));
    orthoProjectionMatrix[13] = -((fTop + fBottom)/(fTop - fBottom));
    orthoProjectionMatrix[14] = -((fFar + fNear)/(fFar - fNear));
