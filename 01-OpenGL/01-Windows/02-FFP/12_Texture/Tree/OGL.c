...
int initialise(void)
{
    ...
    {
        fprintf(gpFile, "loadGLTexture failed for ID_BITMAP_SMILEY\n");
        return -6;
    }
...
BOOL loadGLTexture(GLuint *texture, TCHAR imageResourceID[])
{
    ...
    hBitmap = LoadImage(GetModuleHandle(NULL), "Smiley.bmp", IMAGE_BITMAP, 0, 0,  LR_LOADFROMFILE | LR_CREATEDIBSECTION);
    if (hBitmap)
    {
        bResult = TRUE;
