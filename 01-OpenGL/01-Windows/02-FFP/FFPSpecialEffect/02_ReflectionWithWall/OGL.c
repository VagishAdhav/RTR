...
int initialise(void)
{
    ...
    // load textures
    if (loadGLTexture(&textureMarble, MAKEINTRESOURCE(ID_BITMAP_MARBLE)) == FALSE)
    {
        fprintf(gpFile, "loadGLTexture failed for ID_BITMAP_MARBLE\n");
        return -6;
    }

    if (loadGLTexture(&textureWall, MAKEINTRESOURCE(ID_BITMAP_WALL)) == FALSE)
    {
        fprintf(gpFile, "loadGLTexture failed for ID_BITMAP_WALL\n");
        return -6;
    }
...
void display(void)
{
    ...
    glPushMatrix();
    glTranslatef(-5.0f, 4.0f, -4.0f);
    glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
    glScalef(8.0f, 5.0f, 0.0f);
    drawWall();
    glPopMatrix();
