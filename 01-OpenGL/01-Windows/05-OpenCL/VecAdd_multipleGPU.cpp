...
int main(void)
{
    ...
    // get number if supported platform IDs
    ...
    {
        printf("clGetPlatformIDs() no platforms found : %d\n", numPlatforms);
        cleanup();
        exit(EXIT_FAILURE);
    }
    printf("Supported OpenCL Platforms : %d\n", numPlatforms);
    // allocate memory for platform Ids
    oclPlatformIDs = (cl_platform_id *)malloc(sizeof(cl_platform_id) * numPlatforms);
    // get platform IDs
    result = clGetPlatformIDs(numPlatforms, oclPlatformIDs, NULL);
    if (result != CL_SUCCESS)
    {
        printf("clGetPlatformIDs() Failed : %d\n", result);
        cleanup();
    ...
    for (int platformNum = 0; platformNum < numPlatforms; platformNum++)
    {
        char *platFormName;
        ...
        // get size of the platform name
        result = clGetPlatformInfo(oclPlatformIDs[platformNum], CL_PLATFORM_NAME, 0, NULL, &platformNameSize);
        if (result != CL_SUCCESS)
        {
            printf("clGetPlatformInfo() Failed : %d\n", result);
            cleanup();
            exit(EXIT_FAILURE);
        }

        platFormName = (char *)malloc(sizeof(char) * size);
        // read the the platform name
        result = clGetPlatformInfo(oclPlatformIDs[platformNum], CL_PLATFORM_NAME, platformNameSize, platFormName, NULL);
        if (result != CL_SUCCESS)
        {
            printf("clGetPlatformInfo() Failed : %d\n", result);
            cleanup();
        ...
        printf("Performing Vector Addition on : %s\n\n", platFormName);

        // get OpenCL supporting GPU device's ID
        //cl_uint dev_num;
        result = clGetDeviceIDs(oclPlatformIDs[platformNum], CL_DEVICE_TYPE_GPU, 1, &oclDeviceID, NULL);
        if (result != CL_SUCCESS)
        {
    ...
    // stop timer
        ...
        char str[128];
        if (bAccuracy == false)
            sprintf(str, "Comparison of CPU and GPU vector addition is not within the accuracy of 0.000001 at aray index : %d", breakValue);
        else
