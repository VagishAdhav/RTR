...
struct MyPoint
{
    int x;
    int y;
}vva_point; //declaring a single variable of type 'struct MyPoint' globally
...
}vva_point_properties; //declaring a single variable of type 'struct MyPointProperties' globally
...
int main(void)
{
    ...
    else if (vva_point.x == 0) // If 'X' is zero then 'Y' is the non zero value
    {
        if (vva_point.y < 0) // Y is -ve
            strcpy(vva_point_properties.axis_location, "Negative Y");
        if (vva_point.y > 0) // Y is +ve
            strcpy(vva_point_properties.axis_location, "Positive Y");
    ...
    else if (vva_point.y == 0) // If 'Y' is zero then 'X' is the non zero value
    {
        if (vva_point.x < 0) // X is -ve
            strcpy(vva_point_properties.axis_location, "Negative X");
        if (vva_point.x > 0) // X is +ve
            strcpy(vva_point_properties.axis_location, "Positive X");
    ...
    else // BOTH X and Y are non zero
    {
        ...
        if (vva_point.x > 0 && vva_point.y > 0) // X and Y positive
            vva_point_properties.quadrant = 1;
        if (vva_point.x < 0 && vva_point.y > 0) // X is negative  and Y is positive
            vva_point_properties.quadrant = 2;
        if (vva_point.x < 0 && vva_point.y < 0) // X and Y negative
            vva_point_properties.quadrant = 3;            
        if (vva_point.x > 0 && vva_point.y < 0) // X is positive and Y is negative
