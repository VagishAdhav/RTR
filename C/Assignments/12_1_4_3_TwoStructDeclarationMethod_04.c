...
int main(void)
{
    ...
    // defining struct 
    struct MyPoint vva_point; //declaring a single variable of type 'struct MyPoint' locally

    //defining struct 
    struct MyPointProperties vva_point_properties; //declaring a single variable of type 'struct MyPointProperties' locally
    ...
    else // BOTH X and Y are non zero
    {
        ...
        if (vva_point.x > 0 && vva_point.y > 0) // X +ve and Y +ve
