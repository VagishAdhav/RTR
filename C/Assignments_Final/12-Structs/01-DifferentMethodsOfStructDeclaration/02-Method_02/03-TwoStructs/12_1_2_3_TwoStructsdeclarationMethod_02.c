...
struct MyPoint point_vva; //declaring a single variable of type 'struct MyPoint' globally
struct MyPointProperties point_properties_vva; //declaring a single variable of type 'struct MyPointProperties' globally
...
int main(void)
{
    ...
    else if (point_vva.x == 0) // If 'X' is zero then 'Y' is the non zero value
    {
        if (point_vva.y < 0) // Y is -ve
            strcpy(point_properties_vva.axis_location, "-veY");
        if (point_vva.y > 0) // Y is +ve
            strcpy(point_properties_vva.axis_location, "+ ve Y");
    ...
    else if (point_vva.y == 0) // If 'Y' is zero then 'X' is the non zero value
    {
        if (point_vva.x < 0) // X is -ve
            strcpy(point_properties_vva.axis_location, "-veX");
        if (point_vva.x > 0) // X is +ve
            strcpy(point_properties_vva.axis_location, "+ ve X");
    ...
    else // BOTH X and Y are non zero
    {
        ...
        if (point_vva.x > 0 && point_vva.y > 0) // X and Y +ve
            point_properties_vva.quadrant = 1;
        if (point_vva.x < 0 && point_vva.y > 0) // X is -ve and Y is +ve
            point_properties_vva.quadrant = 2;
        if (point_vva.x < 0 && point_vva.y < 0) // X and Y negative
            point_properties_vva.quadrant = 3;            
        if (point_vva.x > 0 && point_vva.y < 0) // X is + ve and Y is negative
