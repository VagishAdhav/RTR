...
int main(void)
{
    ...
    // defining struct 
    struct MyPoint point_vva; //declaring a single variable of type 'struct MyPoint' locally

    //defining struct 
    struct MyPointProperties point_properties_vva; //declaring a single variable of type 'struct MyPointProperties' locally
    ...
    else // BOTH X and Y are non zero
    {
        ...
        if (point_vva.x > 0 && point_vva.y > 0) // X +ve and Y +ve
